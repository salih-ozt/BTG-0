...
async function sendFcmPush(userId, { title, body, data = {} }) {
    ...
    try {
        ...
        if (!rows || rows.length === 0) {
            // 🔧 DÜZELTİLDİ: Kullanıcı henüz mobil uygulama kurmamış olabilir (web only).
            // Bu normal bir durumdur — warn yerine debug seviyesinde log (production gürültüsü azaltılır).
            if (process.env.FCM_DEBUG === 'true') {
                console.log(`[FCM] userId=${userId} için kayıtlı android/ios/web_fcm token yok (web-only kullanıcı).`);
            }
            return;
        }
        ...
        // Sonuçları logla — sadece başarısız varsa
        const successCount = response.responses.filter(r => r.success).length;
        const failCount    = response.responses.filter(r => !r.success).length;
        if (failCount > 0 && process.env.NODE_ENV !== 'production') {
            // Production'da sadece FCM_DEBUG=true ise logla
            console.warn(`[FCM] userId=${userId} → ${successCount} başarılı, ${failCount} başarısız`);
        } else if (failCount > 0 && process.env.FCM_DEBUG === 'true') {
            console.warn(`[FCM] userId=${userId} → ${successCount} başarılı, ${failCount} başarısız`);
        }

        // Geçersiz token'ları temizle — Promise.all ile paralel, hata yakalandı
        ...
        response.responses.forEach((r, i) => {
            if (!r.success) {
                ...
                if (
                    ...
                    // 🔧 DÜZELTİLDİ: Geçersiz tokenı hem isActive=FALSE yap hem de
                    // yeni kayıt geldiğinde çakışmasın diye token'ı NULL'a çek değil,
                    // sadece isActive kapat (token değeri UNIQUE constraint için tutulur).
                    cleanupPromises.push(
                        dbRun(`UPDATE device_tokens SET "isActive" = FALSE, "updatedAt" = NOW() WHERE token = $1`, [tokens[i]]).catch(() => {})
                    );
                }
            }
        });
...
const MAGIC_SIGNATURES = {
    ...
    // 🔧 EKLENDİ: 3GPP (Android kamera varsayılan formatı) — ftyp box offset 4
    'video/3gpp'  : { offset: 4, bytes: [0x66, 0x74, 0x79, 0x70] },
    // 🔧 EKLENDİ: MPEG — 00 00 01 Bx
    'video/mpeg'  : { offset: 0, bytes: [0x00, 0x00, 0x01] },
    // 🔧 EKLENDİ: Matroska (MKV) — EBML header
...
function mobileKeyMiddleware(req, res, next) {
    ...
    if (!origin && req.path.startsWith('/api/')) {
        // 🔒 GÜVENLİK: null-origin API isteklerinde X-Mobile-App-Key zorunlu.
        ...
        if (expectedKey && (!mobileKey || mobileKey !== expectedKey)) {
            // 🔧 DÜZELTİLDİ: Multipart/form-data (video/resim upload) isteklerinde
            // Android Retrofit bazı durumlarda custom header'ları multipart ile birlikte
            // göndermez. JWT Bearer token varsa kimlik doğrulaması authenticateToken'a bırakılır.
            // Eksik/yanlış key → yalnızca Authorization header'ı da yoksa reddet.
            const hasBearer = req.headers['authorization']?.startsWith('Bearer ');
            const isMultipart = (req.headers['content-type'] || '').includes('multipart/form-data');
            if (!(hasBearer && isMultipart)) {
                return res.status(403).json({ error: 'Erişim reddedildi' });
            }
            // JWT'li multipart isteği: key yok ama token var → devam et, auth middleware yetkilendirir.
        }
        // Multipart dışı JSON yanıtlar için Content-Type zorla (mobileKeyMiddleware'in üstüne yazılmış olabilir)
...
// 🔧 DÜZELTİLDİ: Cloudflare her istekte farklı edge IP gösterebilir.
// Cluster modunda worker başına ayrı in-memory map → farklı workerlar farklı IP sayar.
// 30 limit çok düşük → meşru kullanıcılar SESSION_ANOMALY alıp logout oluyordu.
// 200 ile Cloudflare edge + mobil hat değişimi + cluster fan-out rahatlıkla karşılanır.
const TOKEN_MAX_DISTINCT_IPS = 200;
const TOKEN_MAX_REQUESTS     = 5000;           // 60dk icinde 5000+ istek = suphelidev (cluster/worker bölünmesi hesaba katıldı)
