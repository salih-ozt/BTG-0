...
const UPLOAD_CONFIG = {
    maxFileSize: 100 * 1024 * 1024,    // ✅ 100MB - büyük dosya desteği
...
const upload = multer({
    storage,
    limits: {
        fileSize: 100 * 1024 * 1024, // 100MB limit - büyük dosya desteği
...
// ⚠️ IMPORTANT: Multipart/form-data isteklerinde express.json çalışmamalı
// Aksi halde büyük dosya yüklemeleri başarısız olur
app.use((req, res, next) => {
    // Multipart isteklerde express.json ve urlencoded'ı atla - multer halleder
...
// Post görüntüleme sayacı artır
app.post('/api/posts/:id/view', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        await db.run('UPDATE posts SET views = views + 1 WHERE id = ? AND isActive = 1', id);
        const post = await db.get('SELECT views as viewCount FROM posts WHERE id = ?', id);
        res.json({ viewCount: post ? post.viewCount : 0 });
    } catch (error) {
